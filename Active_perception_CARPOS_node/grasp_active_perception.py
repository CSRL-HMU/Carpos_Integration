...
##############################################################################
def orientation_look_at_center(t,phi_0, phi_1, theta_0, theta_1, p_center, radius):
    ...
    p_des, v_des,_ = polar_position_and_velocity(t,phi_0, phi_1, theta_0, theta_1, p_center, radius)
...
def alpha_profile(t):
    ...
    global features, confidence, P, KALMAN_GAIN, H, Q, F, x, z, listener, stop_signal, measurement_uncertainty, duration
...
def polar_position_and_velocity(t,phi_0, phi_1, theta_0, theta_1, p_center, radius):
    ...
    # polar angles
    phi_t   = phi_0   + alpha*(phi_1   - phi_0)
    theta_t = theta_0 + alpha*(theta_1 - theta_0)
    # derivatives
    phi_dot   = (phi_1 - phi_0)*alpha_dot
    theta_dot = (theta_1 - theta_0)*alpha_dot
...
def desired_pose_polar_with_look_at(t,phi_0, phi_1, theta_0, theta_1, p_center, radius):
    ...
    R_des, w_des = orientation_look_at_center(t,phi_0, phi_1, theta_0, theta_1, p_center, radius)
...
# p_center = np.array([-0.57, 0.148, 0.8])
...
############ !!!!!!!!!!!!!! Vision has to be initiated previously !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
def get_features_from_vision(data):
    global features, confidence, P, KALMAN_GAIN, H, Q, F, x, z, listener, stop_signal, measurement_uncertainty

    for i in range(6):
        features[i*3 : i*3 + 3] = data.data[i*4 : i*4+3]
        confidence[i] = data.data[i*4+3]
    # print(features)
    # print(confidence)
