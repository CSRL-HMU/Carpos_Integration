...
def kinReaching_SE3(p, A, pT, AT, J, kr):
    ...
    qdot = np.linalg.pinv(J , 0.1) @ ( - kr * e )
    return qdot
...
def kinTracking_SE3(p, A, pd, Ad, pd_dot, omegad, J):
    ...
    qdot = np.linalg.pinv(J , 0.1) @ ( v - kt * e )
    return qdot
